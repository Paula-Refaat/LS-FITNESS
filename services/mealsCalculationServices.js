...
//       _id: meal._id,
//       title_AR: meal.title.ar,
//       title_EN: meal.title.en,
...
exports.calculateMeal = asyncHandler(async (req, res, next) => {
  ...
  // 2- Get the language from the request, default to 'en' if not provided
  const lang = req.query.lang || "en"; // Alternatively, you can check `req.body.lang` if needed.
  // 3- Calculate nutritional values based on provided quantities
  try {
    const calculatedNutritionalValues = {
      _id: meal._id,
      title: meal.title[lang] || meal.title.en, // Title based on language
      mealCategory: meal.mealCategory,
      image: meal.image,
      calculationData: {
        Calories: calculateNutritionalValue(
    ...
    // 4- Return the calculated values based on the language
    res.status(200).json({ data: calculatedNutritionalValues });
  } catch (error) {
    return next(new ApiError("Error calculating nutritional values", 500));
  }
