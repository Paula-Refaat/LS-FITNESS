...
exports.createTrainerOrder = async (req, res, next) => {
  ...
  if (profile.user.toString() === req.user.id.toString()) {
    return next(new ApiError("You can't subscribe to your own plans", 400));
  }
  const selectedPlan = profile.plans.find(
    (plan) => plan.type === planType && plan.name === planName
  );
  if (!selectedPlan) {
          ...
          items: [
            {
              ...
              description: `Subscription for ${selectedPlan.name} (${selectedPlan.type})`,
              quantity: 1,
              unit_amount: {
                currency_code: "USD",
                value: planPrice,
  ...
  await Order.create({
    ...
    paymentMethodType: "payPal",
    coupon: couponDoc
      ? { name: couponDoc.name, discount: couponDoc.discount }
      : undefined,
    isPaid: false,
    paypalToken: response.data.id,
  });
...
exports.captureTrainerPlanPayment = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    console.log("Order Response Status:", orderResponse.data.status);

    if (!["APPROVED", "COMPLETED"].includes(orderResponse.data.status)) {
      return res.status(400).json({
        message:
          "Payment has not been approved or completed yet. Please complete the payment before attempting capture.",
    ...
    console.log("Capture Response Status:", captureResponse.data.status);
    ...
    if (captureResponse.data.status === "COMPLETED") {
      ...
      const isSubscribed = trainerProfile.subscribers.some(
        (subscriber) => subscriber.user.toString() === order.user._id.toString()
      );
      ...
      await TrainerProfile.updateOne(
        { _id: trainerProfile._id },
        {
          $push: {
            subscribers: {
              user: order.user._id,
              plan: { name: order.planName, type: order.planType },
              startDate: new Date(),
              endDate,
            },
          },
          $set: {
            totalTrainees: trainerProfile.subscribers.length + 1,
          },
        },
        { session }
      );
      ...
      res.status(200).json({
        ...
        orderId: order._id,
        // trainerPlanId: order.trainerPlan._id,
        // userId: order.user._id,
      });
    } else {
      res.status(400).json({ message: "Payment not successful" });
