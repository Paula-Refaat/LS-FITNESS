...
//     totalPrice: coursePrice,
//     paymentMethodType: "payPal",
//     coupon: coupon ? { name: coupon, discount: couponDoc.discount } : undefined,
//     isPaid: false,
//     paypalToken: response.data.id, // Save the PayPal order ID here
//   });
...
//     if (response.data.status === "COMPLETED") {
//       // Step 3: Find the order by PayPal token and mark it as paid
//       let order = await Order.findOne({ paypalToken, isPaid: false });
//       if (!order) {
//         return next(new ApiError("Order not found or already paid", 404));
//       }
//       order = await Order.findByIdAndUpdate(
...
//         return next(
//           new ApiError(
//             "You are already in this course, we refund your transaction",
//             400
//           )
//         );
//       }
